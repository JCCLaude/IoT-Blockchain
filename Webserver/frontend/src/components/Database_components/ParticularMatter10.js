...
export default class ParticularMatter10 extends Component {
  ...
  deletePM10(id) {
    axios.delete("http://localhost:5000/pm10/" + id).then((response) => {
  ...
  getdata() {
    return this.state.pm10.map((currentpm10) => {
      pm10bv = JSON.stringify(
        <PM10 pm10={currentpm10} key={currentpm10._id} />,
        getCircularReplacer()
      );
      pm10bd = pm10bv.slice(
        pm10bv.indexOf("pm10date"),
        pm10bv.indexOf("pm10geo")
      ); //dates
      pm10bv = pm10bv.slice(
        pm10bv.indexOf("pm10val"),
        pm10bv.indexOf("pm10date")
      ); //values
      ...
      <div>
        ...
        <div class="flex-container" id="legendbox">
          <div id="boxgreen">
            ...
            <p>
              PM10 value {">"} {pm10limitgreen} and {"<"} {pm10limitorange}
            </p>{" "}
          </div>
          <div id="boxorange">
            {" "}
            ...
            <p>
              PM10 value {">"} {pm10limitorange} and {"<"} {pm10limitred}
            </p>{" "}
          </div>
          <div id="boxred">
            {" "}
