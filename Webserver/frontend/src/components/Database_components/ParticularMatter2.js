...
var infotext =
  "This page displays the measured particular matter (PM2.5) emissions. PM2.5 is...";
var Arrayval = [];
...
const PM2 = (props) => (
  <tr>
    ...
    <td
      id={
        props.pm2.pm2val >= pm2limitred
          ? "valuesred"
          : "valuesgreen" &&
            props.pm2.pm2val >= pm2limitorange &&
            props.pm2.pm2val < pm2limitred
          ? "valuesyellow"
          : "valuesgreen"
      }
    ...
    <td>
      <a
        href={"https://maps.google.com/?q=" + props.pm2.pm2geo}
        target="_blank"
...
export default class ParticularMatter2 extends Component {
  ...
  deletePM2(id) {
    axios.delete("http://localhost:5000/pm2/" + id).then((response) => {
  ...
  PM2List() {
    return this.state.pm2
      .map((currentpm2) => {
        return (
          <PM2
            pm2={currentpm2}
            deletePM2={this.deletePM2}
            key={currentpm2._id}
          />
        );
      })
  ...
  getdata() {
    return this.state.pm2.map((currentpm2) => {
      pm2bv = JSON.stringify(
        <PM2 pm2={currentpm2} key={currentpm2._id} />,
        getCircularReplacer()
      );
      ...
      <div>
        ...
        <div class="flex-container" id="legendbox">
          <div id="boxgreen">
            ...
            <p>
              PM2.5 value {">"} {pm2limitgreen} and {"<"} {pm2limitorange}
            </p>{" "}
          </div>
          <div id="boxorange">
            {" "}
            ...
            <p>
              PM2.5 value {">"} {pm2limitorange} and {"<"} {pm2limitred}
            </p>{" "}
          </div>
          <div id="boxred">
            {" "}
