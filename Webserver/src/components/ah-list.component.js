...
var infotext = "This page displays the measured Air Humidity. Air Humidity is...";
var Arrayval = [];
...
<tr>
    ...
    <td id={props.ah.ahval >= ahlimitred ? 'valuesred':'valuesgreen' && props.ah.ahval >= ahlimitorange && props.ah.ahval < ahlimitred ? 'valuesyellow':'valuesgreen'}> {props.ah.ahval}</td> {/*{props.co.coval} {if(props.co.coval >= 800) {id='covaluesgreen'}}*/}
    <td>
      <a href={"https://maps.google.com/?q="+props.ah.ahgeo} target="_blank" rel="noopener noreferrer">Maps</a>
    </td>
  </tr>
)
...
export default class AHList extends Component {
  constructor(props) {
    super(props);
  ...
  deleteAH(id) {
    axios.delete('http://localhost:5000/ah/'+id)
  ...
  AHList() {
    return this.state.ah.map(currentah => {
      return <AH ah={currentah} deleteAH={this.deleteAH} key={currentah._id}/>;
    }).reverse()
  }

  getdata() {
    return this.state.ah.map(currentah => {
      ahbv = JSON.stringify((<AH ah={currentah} key={currentah._id}/>), getCircularReplacer());
      ...
      ahbd1 = ahbd1.replace("T", " ").replace("Z", "").replaceAll('"','').slice(0,ahbd1.indexOf(".")-1);
    ...
    return (
      ...
      <div>
        ...
        <div class='flex-container' id='legendbox'>
          <div id="boxgreen"> <p>Color Green:</p> <br></br> <p>Air Humidity value {'>='} {ahlimitgreen}% and {'<='} {ahlimitorange}%</p> </div>
          <div id="boxorange"> <p>Color Orange: </p> <br></br> <p>Air Humidity value {'>'} {ahlimitorange}% and {'<'} {ahlimitred}%</p> </div> 
          <div id="boxred"> <p>Color Red: </p> <br></br> <p>Air Humidity value {'>='} {ahlimitred}%</p> </div> 
        </div>
